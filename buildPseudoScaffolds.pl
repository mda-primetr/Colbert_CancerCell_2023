...
#construct the pseudoscaffolds for nucleotide and protein alignments
#requires clustering output file from clusterReferences.pl


open REF, "cat $ARGV[0] | cut -f1 |";
my $goodReferences = {};
while (my $line = <REF>) {
	chomp $line;
	if ($line =~ m/^Parent:/) {
	...
	foreach my $read (@reads) {
		unless (exists $bestHitPerRead->{$read}->{$reference}) {
			...
			if (scalar(keys %{$bestHitPerRead->{$read}}) == 1) {
				$soloCount++;
			} elsif (exists $nucBestPerRead->{$read}->{$reference} and scalar(keys %{$nucBestPerRead->{$read}}) == 1) {
				$soloCount++;
			} else {
				delete $reference2Reads->{$reference}->{$read};
			}
...
foreach my $ref (sort keys %$reference2Reads) {
	...
	foreach my $readName (sort {$readInfo->{$a}->{$ref}->{'start'} <=> $readInfo->{$b}->{$ref}->{'start'}} keys %{$reference2Reads->{$ref}}) {
		...
		#browseErr($readInfo->{$readName}->{$ref});
		$readName =~ m/$sizeQR/;
		my $countAdd = $1;
		...
		#print STDERR "dnaStart = $dnaStart\n";
		#print STDERR "adjusted = $adjustedRead\n";
...
foreach my $reference (keys %$nucReferenceToReads) {	
	...
	foreach my $read (keys %{$nucReferenceToReads->{$reference}}) {
		...
		for (my $i = 0; $i < $matchLength; $i++) {
			...
			#if ($directives[$directivePos] eq "M" or $directives[$directivePos] eq "S") {
			if ($directives[$directivePos] eq "M") {
				unless (defined $readLetters[$i]) {
					print STDERR "read = $read, ref = $reference, cigar = $cigar, i = $i\n";
					browseErr(\@readLetters);
				}
