...
#						print STDERR "mismatch position $j($seqArray[$j]) on $parent with $child position $i($childSeqArray[$i]) after reverse\n";
...
#						print STDERR "mismatch position $j($seqArray[$j]) on $parent with $child position $i($childSeqArray[$i])\n";
						$seqArray[$j] = lc($seqArray[$j]);;
					}
					$i++;
				}
